...
export function WizardStageLayout() {
  ...
  const etapaNumero = m ? parseInt(m[1], 10) : 1;
  ...
  const base = `/wizard/${wizardEtapaPath(etapaNumero)}`;
    ...
    <Row gutter={[24, 24]}>
      <Col xs={24} lg={7}>
        <Card
          title={
            <span>
              Etapa {etapaNumero} de 6 <Tag color="blue">{meta.eta}</Tag>
            </span>
          }
